<template>
  <router-link
    @click="readState = false"
    :to="{ name: 'channels', params: { id: channel.id } }"
    active-class="text-white bg-gray-550/[0.32]"
    :class="[
      readClass,
      'flex items-center px-2 mx-2 py-1 rounded group relative active:bg-gray-550/[0.24]'
    ]">
    <div v-if="readState" class="absolute w-1 h-2 bg-white left-0 -ml-2 rounded-r-full"></div>
...
const readState = ref(channel.unread)
